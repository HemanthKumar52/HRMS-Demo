      ...
      child: ResponsiveContent(
        child: Column(
          ...
          children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Command center, users, audit logs & settings',
            ...
            if (role == UserRole.admin) const SizedBox(height: 16),
            ...
            // ── 5. Manager Teams — tap to see full team attendance ─────
            if (isManagerOrHr) ...[
              GestureDetector(
                    onTap: () => Navigator.push(
                      context,
                      Motion.pageRoute(const TeamAttendanceScreen()),
                    ),
                    child: _TeamAttendanceCard(isDark: isDark),
                  )
                  .animate()
            ...
            // ── 5b. Team Performance (right after the team card) ─────────
            if (isManagerOrHr) ...[
            ...
            Builder(
              builder: (context) {
                ...
                if (provider.leaveBalances.isEmpty)
                  totalRemaining = provider.leaveBalance;
                return Row(
                      children: [
                        Expanded(
                          child: NeuCard(
                    ...
                    .fadeIn(
                      duration: 420.ms,
                      delay: (isManagerOrHr ? 640 : 400).ms,
                    )
                    .slideY(
                      begin: 0.12,
                      end: 0,
            ...
            NeuCard(
                ...
                .slideY(
                  ...
                  duration: 420.ms,
                  delay: (isManagerOrHr ? 720 : 480).ms,
                  curve: Curves.easeOutCubic,
                ),
            const SizedBox(height: 16),
            ...
            NeuCard(
                ...
                .slideY(
                  ...
                  duration: 420.ms,
                  delay: (isManagerOrHr ? 880 : 560).ms,
                  curve: Curves.easeOutCubic,
                ),
            const SizedBox(height: 16),
            ...
            Text('Recent Activity', style: theme.textTheme.titleMedium)
                ...
                .slideY(
                  ...
                  duration: 420.ms,
                  delay: (isManagerOrHr ? 960 : 640).ms,
                  curve: Curves.easeOutCubic,
                ),
            const SizedBox(height: 12),
            NeuCard(
                ...
                .fadeIn(
                  duration: 420.ms,
                  delay: (isManagerOrHr ? 1040 : 720).ms,
                )
                .slideY(
                  begin: 0.12,
                  end: 0,
        ...
        NeuCard(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  ..._items.expand((spec) sync* {
                    yield _InsightActionRow(
                      label: spec.label,
                      count: ((pa[spec.key] ?? 0) as num).toInt().toString(),
                      color: spec.color,
                      onTap: () => _navigateToRequested(context),
                    );
                    if (spec != _items.last) yield const SizedBox(height: 10);
                  }),
                ],
              ),
            )
...
class _InsightSpec {
  ...
  final String label;
  final Color color;
  const _InsightSpec(this.key, this.label, this.color);
}
...
class _TeamAttendanceCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final provider = context.watch<AppProvider>();
    final team = provider.teamSummary;
    final size = ((team['size'] ?? 0) as num).toInt();
    final today = (team['today'] ?? const {}) as Map;
    final present = ((today['present'] ?? 0) as num).toInt();
    final wfh = ((today['wfh'] ?? 0) as num).toInt();
    final onLeave = ((today['on_leave'] ?? 0) as num).toInt();
    final absent = ((today['absent'] ?? 0) as num).toInt();
    final pct = size > 0 ? (present / size).clamp(0.0, 1.0) : 0.0;
    final departments = List<Map<String, dynamic>>.from(
      (team['departments'] ?? const <dynamic>[]) as List,
    );
    final workTypes = List<Map<String, dynamic>>.from(
      (team['work_types'] ?? const <dynamic>[]) as List,
    );
              ...
              child: Column(
                ...
                children: [
                  ...
                  _PerformanceRow(
                    ...
                    pct: attPct,
                    pctText: '${_attendanceRate.toStringAsFixed(1)}%',
                    color: AppColors.success,
                  ...
                  _PerformanceRow(
                    ...
                    pct: hoursPct,
                    pctText: '${_avgHours.toStringAsFixed(1)}h',
                    color: AppColors.primary,
                    isDark: widget.isDark,
                  ),
                ],
    ...
    try {
      ...
      setState(() {
        ...
        _leaveUtil = List<Map<String, dynamic>>.from(
          (data['leave_utilization'] as List?)?.map(
                    ...
                    ..._leaveUtil.asMap().entries.map((entry) {
                      ...
                      return Padding(
                        padding: EdgeInsets.only(
                          bottom: i < _leaveUtil.length - 1 ? 14 : 0,
                        ),
                        child: _LeaveUtilRow(
                          label: l['label'] ?? '',
...
class _TeamSplitChip extends StatelessWidget {
  final String label;
  final int count;
  final Color color;
  final bool isDark;
  const _TeamSplitChip({
    required this.label,
    required this.count,
    required this.color,
    required this.isDark,
